...
typedef pair< pair<int, int>, pair<int, int> > pop;

void showQueries( vector<pop>& queries)
{
  for( int i=0; i<queries.size(); i++)
  cout << queries[i].F.F << " " << queries[i].F.S << endl;
}
...
int main( int argc, char** argv)
{
  ...
  {
    pop temp;
    temp.F.F = input.readInt();
    temp.F.S = input.readInt();
    temp.S.F = i;
    temp.S.S = 0;
