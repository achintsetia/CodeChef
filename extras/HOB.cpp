...
int main( int argc, char** argv)
{
  ...
  while( T--)
  {
    ...
    for( int i=0; i<N; i++)
    {
      guestList[i].arrival = 60*input.readInt() + input.readInt();
      guestList[i].stay = 60*input.readInt();
      ...
      while( c != '\n')
      {
        guestList[i].name[j++] = (c >= 97) ? c : c+32;
        c = input.readChar();
      }
    }
    ...
    for( int i=0; i<N; i++)
    {
      cout << guestList[i].name << " " << guestList[i].arrival << " ";
      cout << guestList[i].stay << endl;
    }
