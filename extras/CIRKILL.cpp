...
void getCircle( pair<int, int> A, pair<int, int> B, pair<int, int> C, double circle[])
{

}

bool inCircle( double circle[], pair<int, int> p)
{
  return ((p.f + circle[0])*p.f + (p.s + circle[1])*p.s + circle[2]) <= 0.0;
}
...
int main( int argc, char** argv)
{
  ...
  while( T--)
  {
    ...
    int N = input.readInt();
    for( int i=0; i<N; i++)
      coord.push_back(make_pair(input.readInt(), input.readInt()));
    ...
    for( int i=0; i<N; i++)
      for( int j=i+1; j<N; j++)
        for( int k=j+1; j<N; j++)
        {
          getCircle( coord[i], coord[j], coord[k], circle);

          for( int p=0; p<N; p++)
            if( p!=i && p!=j && p!=k && inCircle(circle, coord[p]))
              positive++;
        }

    printf( "%0.7f\n", (double) positive / (double) (N*(N-1)*(N-2)*(N-3)/6));
  }
  return 0;
}
