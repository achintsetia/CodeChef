...
{
  double dArea = (double) ((x1 - x0)*(y2 - y0) - (x2 - x0)*(y1 - y0))/2.0;
...
int main( int argc, char** argv)
{
  ...
  for( int i=1; i<=N; i++)
  {
    arr[i] = area(input.readInt(), input.readInt(), input.readInt(), input.readInt(), input.readInt(), input.readInt());
    if( arr[i] <= minArea)
    {
      minArea = arr[i];
      minIndex = i;
