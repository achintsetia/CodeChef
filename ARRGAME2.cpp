...
void solve(int N)
{
  sort( logB.begin(), logB.end());
  sort( logG.begin(), logG.end());

  double ans = 0.0;
  vector<double> :: iterator ub,lb;
  int lessThan;
  for( int i=N-1; i>=0; i--)
  {
    lessThan = lower_bound(logG.begin(), logG.end(), logB[i]) - logG.begin();
    ans += (double) lessThan/(double) N;
  }
...
int main( int argc, char** argv)
{
  ...
  while(T--)
  {
    ...
    {
      B = input.readInt();
      logB.push_back(log2((double)B) / (double) B);
    }
    for( int i=0; i<N; i++)
    {
      G = input.readInt();
      logG.push_back(log2((double)G) / (double) G);
    }
