...
int main( int argc, char** argv)
{
  ...
  for( int i=0; i<MAX; i++)
    if( B[i] >= 1) arr.push_back(i);

  int start = 1;

  selVec.push_back(arr[start]);
  selected[arr[start]] = 1;

  int arrS = arr.size();
  int count = 1;

  for( int i=start+1; i<arrS; i++)
  {
    bool toSelect = true;
    for( int j=count-1; j>=0; j--)
    {
      int diff = arr[i] - selVec[j];
      if( (selVec[j] - diff) >= 0 && selected[selVec[j] - diff] == 1)
      {
        toSelect = false;
        break;
      }
