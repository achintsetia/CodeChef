...
double table[2000][2000];

void calcTree( int end)
{
  if( end > 1)
  {
    table[end-1][0] = 1.0/2.0;
    table[end-1][end-1] = 1.0/2.0;
    ...
    for( int i=0; i<end-2; i++)
      table[end - 1][i] += (table[end-1-2][i])/4.0;

    for( int i=2; i<end; i++)
      table[end - 1][i] += (table[end-1-2][i-2])/4.0;

    for( int i=1; i<end-1; i++)
      table[end - 1][i] += (table[end-1-2][i-1])/2.0;
...
int main( int argc, char** argv)
{
  ...
  while( T--)
  {
    ...
    for( int i=0; i<N; i++)
      expSum += table[N-1][i] * input.readInt();
