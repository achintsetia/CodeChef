...
int diffSum( int H[], int D[], int N)
{
  int sum = 0;
  for( int i=0; i<N; i++) sum += D[i] - H[i];
  return sum;
}
bool isEqual( int H[], int D[], int N)
{
  for( int i=0; i<N; i++) if( D[i] != H[i]) return false;
  return true;
}

int solve(int H[], int D[], int N)
{
  int sum = diffSum( H,D,N);

  if( N == 2)
  {
    if(sum == 0) return max(D[0] - H[0], H[0] - D[0]);
    else return -1;
  }
  ...
  int count = 0;
  while( diffSum(H,D,N) >=0 && !isEqual(H,D,N))
  {
    count++;
    int Min = D[0] - H[0];
    int minIndex = 0;
...
int main( int argc, char** argv)
{
  ...
  while(T--)
  {
    ...
    int H[N], D[N];
    for( int i=0; i<N; i++) H[i] = input.readInt();
    for( int i=0; i<N; i++) D[i] = input.readInt();
