...
long long int powInt( int num, int p)
{
  ...
  else return (num * powInt(num, p-1)) % MOD;
}
...
int main( int argc, char** argv)
{
  ...
  while(T--)
  {
    ...
    {
      int D = 2*N-M-1;
      long long int base = factorials[N-D];
      long long int rCurr = (N%2==0)? 2*((M-N+1)/2):2 * ((M-N)/2)+1;
      long long int rPrev = D%2 == 0 ? rCurr-1 : rCurr+1;
      ...
      {
        ans = (base * powInt(rCurr, D/2)) % MOD;
        ans = (ans * powInt(rPrev, D/2)) % MOD;
      }
      else
      {
        ans = (base*rCurr) % MOD;
        ans = (ans * powInt(rCurr, D/2)) % MOD;
        ans = (ans * powInt(rPrev, D/2)) % MOD;
      }
      output( ans, "\n");
    }
